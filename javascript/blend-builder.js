...
/* Rank a format code by its position in this role's ladder (for sorting catalogue results within a tier).
   Buckets (ok/warn/avoid) are sorted separately, so a plain array index suffices; unlisted formats sort last. */
function ladderRank(c,code){
 if(!c) return 9999;
 var lists=[c.preferred_formats,c.conditional_formats,c.unsuitable_formats];
 for(var i=0;i<lists.length;i++){var a=lists[i];if(a&&a.length){var ix=a.indexOf(code);if(ix>-1)return ix;}}
 return 9999;
}
