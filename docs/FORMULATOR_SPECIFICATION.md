...
1. **11 ladder rows** where the proposed best-fit order changes the displayed recommendation
   (see `matrix/Stage1_Format_Ladder_AUDIT.xlsx`).
...
Authoritative data artifacts live in `matrix/`:
...
The parity report lives in `docs/herbuno-matrix_PARITY_REPORT.md` — parity for the last matrix apply.
