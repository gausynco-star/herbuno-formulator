...
- [ ] **Unrecognised free-text ingredient** (e.g. "Zzyzx fictus") → no crash and the Product × Role
      ...
      (or equivalent)** — the tool must not imply the botanical exists or has been validated.
      *(Implemented in f6fe9de — verify it renders live.)*
...
- [ ] Existing **Build-mode** Add-to-cart remains functional and unchanged (Step 1 didn't touch
      Build mode). *(Stage-2 SKU-specific Add-to-cart is NOT implemented in Step 1 — do not test it
      here; see future procurement-view requirements.)*
...
- [ ] **When no catalogue match exists, "Request sample" is hidden or clearly reframed as a
      ...
      presently stocked. *(Implemented in f6fe9de — "Request a sourcing sample" on no-match; verify
      it renders live.)*
...
## Honesty fixes already applied (commit f6fe9de) — verify they render live
1. **Unverified botanical identity** — off-list names render as "<name> — unverified botanical name"
   (muted italic); format guidance unaffected, identity qualified. (North Star: a *defensible* spec,
   not merely a valid object.)
2. **"Request sample" on a no-match** — becomes "Request a sourcing sample" when nothing matching is
   in the catalogue (gated on `CAT_LOADED && (exact || altOk)`), so it never implies stock exists.

## Future procurement-view requirements (later step, not Step 1)
- Exact catalogue matches expose SKU-specific **Add-to-cart** and a **stocked-sample** action
  (distinct from the "Request a sourcing sample" enquiry already built for no-match cases).
